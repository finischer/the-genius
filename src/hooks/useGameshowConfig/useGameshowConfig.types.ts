...
export type IUseGameshowConfigReturn<T extends Game, V> = GameConfigReturn<
  T,
  V
> & {
  ...
  updateGame: T extends undefined
    ? undefined
    : (updateFn: (config: TGameSettingsMap[T]) => void) => void;
};

export type TGameConfigUpdateFn<T extends Game> = (
